						...
						<div class="panel-body"> 
							<div class="row">
								<div class="col-md-12">
									...
									<table class="table table-bordered table-hover" id="time-slots-table">
										<thead>
											<th>#</th>
											<th>Time From</th>
											<th>Time To</th>
...
@section('scripts')
	<script>
		 $(function(){
		    var table = $('#time-slots-table').DataTable({
		      bProcessing: true,
		      bServerSide: false,
		      sServerMethod: "GET",
		      'ajax': '/admin/get-time-slots',
		      searching: true, 
		      paging: true, 
		      filtering:false, 
		      bInfo: true,
		      ...
		      "columns": [ 
		        {data: 'row',  name: 'row', className: ' text-left hidden',   searchable: true, sortable: true}, 
		        {data: 'from',  name: 'from', className: 'col-md-4 text-center',  searchable: true, sortable: true},  
		        {data: 'to',  name: 'to', className: 'col-md-4 text-center',  searchable: true, sortable: true},  
		    ...
		    $(document).off('click','.edit-data-btn').on('click','.edit-data-btn', function(e){
		      ...
		      $("#editmodal").modal();
		      $.ajax({
		        url: '/admin/time-slots/'+that.dataset.id+'/edit',         
		        success: function(data) {
		          $("#editmodal").html(data);
		        }
		      }); 
		            ...
		            bootbox.confirm({
		              ...
		              callback: function (result) {
		                 if(result){
		                  ...
		                  $.ajax({
		                  url:'/admin/time-slots/'+that.dataset.id,
		                  type: 'post',
